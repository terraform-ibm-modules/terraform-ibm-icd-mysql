...
resource "ibm_iam_authorization_policy" "backup_kms_policy" {
  ...
  source_resource_group_id = var.resource_group_id
  roles                    = ["Reader"]
  description              = "Allow all MySQL instances in the Resource Group ${var.resource_group_id} to read the ${local.backup_kms_service} key ${local.backup_kms_key_id} from the instance GUID ${local.backup_kms_key_instance_guid}"
  resource_attributes {
    name     = "serviceName"
    operator = "stringEquals"
...
resource "ibm_resource_tag" "mysql_tag" {
  count       = length(var.access_tags) == 0 ? 0 : 1
  resource_id = ibm_database.mysql_db.resource_crn
...
resource "ibm_resource_key" "service_credentials" {
  ...
  name                 = each.key
  role                 = each.value
  resource_instance_id = ibm_database.mysql_db.id
}
...
locals {
  ...
  service_credentials_object = length(var.service_credential_names) > 0 ? {
    hostname    = ibm_resource_key.service_credentials[keys(var.service_credential_names)[0]].credentials["connection.mysql.hosts.0.hostname"]
    certificate = ibm_resource_key.service_credentials[keys(var.service_credential_names)[0]].credentials["connection.mysql.certificate.certificate_base64"]
    port        = ibm_resource_key.service_credentials[keys(var.service_credential_names)[0]].credentials["connection.mysql.hosts.0.port"]
    credentials = {
      for service_credential in ibm_resource_key.service_credentials :
      service_credential["name"] => {
        username = service_credential.credentials["connection.mysql.authentication.username"]
        password = service_credential.credentials["connection.mysql.authentication.password"]
      }
    }
  } : null
}
