...
#   - Gen2 requires account-level S2S policy (no resource group scope)
#######################################################################################################################
...
# Create auth policy (scoped to exact KMS key) - Gen2 requires account-level policy
resource "ibm_iam_authorization_policy" "kms_policy" {
  ...
  description = "Allow all MySQL instances in the account ${local.account_id} to read the ${local.kms_service} key ${local.kms_key_id} from the instance GUID ${local.kms_instance_guid}"
  resource_attributes {
    name     = "serviceName"
...
locals {
  ...
  mysql_hostname = var.existing_mysql_instance_crn != null ? null : module.mysql[0].hostname
  mysql_port     = var.existing_mysql_instance_crn != null ? null : module.mysql[0].port
}
