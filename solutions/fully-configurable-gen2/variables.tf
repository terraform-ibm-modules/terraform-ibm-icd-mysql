...
variable "member_host_flavor" {
  ...
  validation {
    ...
    error_message = "Member host flavor must be specified and cannot be 'multitenant' for Gen2 instances. [Learn more](https://registry.terraform.io/providers/IBM-Cloud/ibm/latest/docs/resources/database#host_flavor)."
  }
}

variable "service_credential_names" {
  description = "A list of service credential resource keys to be created for the MySQL instance. [Learn more](https://github.com/terraform-ibm-modules/terraform-ibm-icd-mysql/blob/main/solutions/fully-configurable-gen2/DA-types.md#svc-credential-name)"
  type = list(object({
    name     = string
...
variable "resource_tags" {
  type        = list(string)
  description = "Add user resource tags to the MySQL instance to organize, track, and manage costs. [Learn more](https://cloud.ibm.com/docs/account?topic=account-tag&interface=ui#tag-types)."
  default     = []
}
...
variable "kms_encryption_enabled" {
  type        = bool
  description = "Set to true to enable KMS encryption using customer-managed keys. When enabled, you must provide a value for at least one of the following: existing_kms_instance_crn or existing_kms_key_crn. If set to false, IBM-owned encryption is used (i.e., encryption keys managed and held by IBM). Note: Gen2 instances do not support separate backup encryption keys."
  default     = false
...
variable "existing_kms_instance_crn" {
  type        = string
  description = "The CRN of a Key Protect instance. Required to create a new encryption key and key ring which will be used to encrypt deployment data. To use an existing key, pass a value for `existing_kms_key_crn`. Note: Gen2 instances do not support separate backup encryption keys."
  default     = null
...
variable "existing_kms_key_crn" {
  type        = string
  description = "The CRN of a Key Protect encryption key to encrypt your data. If no value is passed a new key will be created in the instance specified in the `existing_kms_instance_crn` input. Note: Gen2 instances do not support separate backup encryption keys."
  default     = null
...
variable "service_credential_secrets" {
  ...
  default     = []
  description = "Service credential secrets configuration for Databases for MySQL. [Learn more](https://github.com/terraform-ibm-modules/terraform-ibm-icd-mysql/tree/main/solutions/fully-configurable-gen2/DA-types.md#service-credential-secrets)."
